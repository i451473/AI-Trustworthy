...
class Program
{
    static async Task Main()
    {
        ...
        {
            var htmlWarning = BuildHtmlPage("AI Summary Skipped", $"<p>{WebUtility.HtmlEncode(inputWarning)}</p>", null);
        ...
        // --- Retry Helper ---
        // Func<Task<T>> means: 
        // “Give me a function that runs in the background and returns a result.”
        ...
        string finalOutput = $"""
            ...
            **Trust Score: {trustLevel}** (Based on source validation and consistency across runs)
            Source Validation: {sourceConfidenceLevel} | Consistency: {consistencyLevel}
            {reviewNotes}
            [
            Transparency & Accountability:

            This summary and recommendations were generated by AI.
            Based only on the uploaded document: {Path.GetFileName(filePath)}.
            This AI works best with clear, structured reports. It may miss context in complex or poorly formatted documents. Always review before acting.

            Internal Datasheet Notes:
            - Expected documents: internal project or research reports.
            - No personal or external data was used.
            - AI Model: {deploymentName} via Azure OpenAI API.
            ]
            """;

        // Build the HTML page and write it to disk
        var htmlContent = BuildHtmlPage("AI Summary (Trust-Enhanced)", ConvertMarkdownToHtml(finalOutput), new Dictionary<string, string>
        {
            { "File", Path.GetFileName(filePath) },
            { "Model", deploymentName },
            { "GeneratedUtc", DateTime.UtcNow.ToString("u") },
            { "Temperature", "0.1 (summary), 0.0 (validation)" },
        ...
        // Attempt to open in default browser (cross-platform)
        OpenFileInBrowser("report.html");
    }

    // Helper: Convert a simple markdown-ish finalOutput into HTML blocks.
    static string ConvertMarkdownToHtml(string markdown)
    {
        // Very small converter: headers, bold, lists, paragraphs, horizontal separator
        ...
        // Simple bullet points starting with - 
    ...
    static string BuildHtmlPage(string title, string bodyHtml, Dictionary<string, string>? meta = null)
    {
        ...
        if (meta != null)
        {
            metaHtml = "<dl class=\"meta\">\n" + string.Join("\n", meta.Select(kv => $"<dt>{WebUtility.HtmlEncode(kv.Key)}</dt><dd>{WebUtility.HtmlEncode(kv.Value)}</dd>")) + "\n</dl>";
        }
        ...
        var css = @"body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial; margin: 24px; background: #f6f8fa; color: #0b1226; }
            ...
            .meta { float: right; text-align: right; font-size: 13px; color: #42526e; }
            .meta dt { font-weight: 700; }
            .meta dd { margin: 0 0 8px 0; }
            .section { margin-top: 16px; }
            hr { border: none; border-top: 1px solid #e6eef8; margin: 16px 0; }
            pre { background: #0b1226; color: #e6eef8; padding: 12px; border-radius: 8px; overflow: auto; }
            ul { margin: 8px 0 8px 18px; }
            footer { margin-top: 20px; font-size: 13px; color: #526070; }
            ";

        return $"<!doctype html>\n<html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>{WebUtility.HtmlEncode(title)}</title><style>{css}</style></head><body><div class=\"container\"><header><h1>{WebUtility.HtmlEncode(title)}</h1>{metaHtml}</header><main class=\"section\">{bodyHtml}</main><footer><p>Generated by AI on {DateTime.UtcNow:u}</p></footer></div></body></html>";
    }
